    ...
    return (
        <Container maxWidth="lg" style={{ height: '100%' }}>
            <Grid container spacing={2}>
                {data.map(item =>
                    <Grid item xs={2}>
                        <Box style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', backgroundColor: chooseColor(item.index) }}>
                            <Typography style={{ color: 'white' }}>{item.name}</Typography>
                            <Typography style={{ color: 'white' }}>IQA {item.index}</Typography>
                        </Box>
                    </Grid>)}
            </Grid>
        </Container >
